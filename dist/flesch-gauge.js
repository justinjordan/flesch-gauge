...
function create_fragment(ctx) {
	...
	return {
		c: function create() {
			...
			attr(circle, "cx", "60");
			attr(circle, "cy", "60");
			add_location(circle, file, 31, 2, 684);
			attr(svg, "class", "progress-ring");
			attr(svg, "width", "120");
			attr(svg, "height", "120");
			add_location(svg, file, 27, 0, 620);
		},
...
function instance($$self, $$props, $$invalidate) {
	let { radius = 52, percent = 100 } = $$props;
  ...
  let color = 'hwb(0, 0%, 0%)';
	...
	const writable_props = ['radius', 'percent'];
	...
	$$self.$$.update = ($$dirty = { percent: 1, circumference: 1 }) => {
		if ($$dirty.percent || $$dirty.circumference) { {
        // timeout to enable transition
        setTimeout(() => {
          $$invalidate('color', color = 'hwb(' + Math.floor(120 * percent / 100) + ', 0%, 0%)');
...
class Ring extends SvelteComponentDev {
	constructor(options) {
		...
		init(this, options, instance, create_fragment, safe_not_equal, ["radius", "percent"]);
	}
...
function countWords(content) {
  ...
  return content.split(/\s+/).filter(function (el) {
    return !!el;
  }).length;
}
...
function Calculate(content) {
  ...
  var syllables = syllable_1(content);
  var score = flesch_1({
    word: words,
    sentence: sentences,
    syllable: syllables
...
function create_fragment$1(ctx) {
	...
	return {
		c: function create() {
			...
			add_location(p0, file$1, 65, 8, 1492);
			p1.className = "flesch-gauge__rating svelte-pa1i1i";
			add_location(p1, file$1, 66, 8, 1543);
			div0.className = "flesch-gauge__copy svelte-pa1i1i";
			add_location(div0, file$1, 64, 4, 1451);
...
export default App;
//# sourceMappingURL=flesch-gauge.js.map
