...
function create_fragment(ctx) {
	...
	return {
		c: function create() {
			...
			attr(circle, "cx", "60");
			attr(circle, "cy", "60");
			add_location(circle, file, 30, 2, 685);
			attr(svg, "class", "progress-ring");
			attr(svg, "width", "120");
			attr(svg, "height", "120");
			add_location(svg, file, 26, 0, 621);
		},
...
function Calculate(content) {
  const score = flesch_1({sentence: countSentences(content), word: countWords(content), syllable: syllable_1(content)});
...
function create_fragment$1(ctx) {
	...
	return {
		c: function create() {
			...
			t1 = text("Score: ");
			t2 = text(ctx.score);
			add_location(p, file$1, 10, 0, 185);
		},
